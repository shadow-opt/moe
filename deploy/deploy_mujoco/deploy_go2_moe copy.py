...
def get_xbox_command(joystick, max_cmd):
    ...
    if joystick is not None:
        ...
        return np.array([cmd_x, cmd_y, cmd_yaw, 0, 0, 0, 0, 0], dtype=np.float32)
...
if __name__ == "__main__":
    ...
    cmd_str = f"cmd_{cmd[0]}_{cmd[1]}_{cmd[2]}_{cmd[3]}_{cmd[4]}_{cmd[5]}_{cmd[6]}_{cmd[7]}"
    ...
    with mujoco.viewer.launch_passive(m, d) as viewer:
        ...
        while viewer.is_running() and time.time() - start < simulation_duration:
            ...
            if counter % control_decimation == 0:
                ...
                qj_mj = d.qpos[7:]
                dqj_mj = d.qvel[6:]
                quat_mj = d.qpos[3:7]  # [w, x, y, z]
                lin_vel_global = d.qvel[:3]
                ang_vel_local = d.qvel[3:6]

                # 2. MuJoCo 原生角速度已在局部坐标系，不能再做四元数逆旋转

                # 3. 维度对齐：先将 MuJoCo 顺序的关节状态映射回 Isaac Gym 顺序 (解决乱抖)
                qj_model = qj_mj[idx_mj2model]
                dqj_model = dqj_mj[idx_mj2model]

                # 4. 在同一个维度空间内计算偏移量
                qj_obs = (qj_model - default_angles) * dof_pos_scale
                dqj_obs = dqj_model * dof_vel_scale
                gravity_orientation = get_gravity_orientation(quat_mj)

                # 5. 填装观测向量 obs
                obs[:3] = ang_vel_local * ang_vel_scale
                obs[3:6] = gravity_orientation
                obs[6:14] = cmd * cmd_scale
                obs[14 : 14 + num_actions] = qj_obs
                obs[14 + num_actions : 14 + 2 * num_actions] = dqj_obs
